...
@section('content')
<x-common.page-breadcrumb pageTitle="Manajemen Admin" />
...
<div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
    ...
    <div class="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
        <div>
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Manajemen Admin</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400">Kelola akun admin & owner</p>
        </div>
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-gray-200 border-y dark:border-gray-700">
                        ...
                        <th class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Nama</th>
                        <th class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Email</th>
                        <th class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">No. Telp</th>
                        <th class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Role</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                <img src="{{ $a->foto_url ?? asset('images/default-avatar.png') }}"
                                    class="w-8 h-8 rounded-full object-cover">
                                <div>
                                    <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $a->name }}</p>
                                    @if($a->id === auth()->id())
                        ...
                        <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $a->email }}</td>
                        <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $a->noTelp ?? '-' }}</td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            @if($a->role === 'owner')
                            <span class="px-2 py-1 text-xs font-semibold rounded-full bg-purple-50 text-purple-600 dark:bg-purple-500/15 dark:text-purple-400">Owner</span>
                            @else
                            <span class="px-2 py-1 text-xs font-semibold rounded-full bg-blue-50 text-blue-600 dark:bg-blue-500/15 dark:text-blue-400">Admin</span>
                            @endif
                        </td>
                        <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                            <div class="flex justify-center relative">
                                <x-common.table-dropdown>
                                    ...
                                    <x-slot name="content">
                                        <button onclick="editAdmin({{ $a->id }}, '{{ addslashes($a->name) }}', '{{ $a->email }}', '{{ $a->noTelp }}', '{{ $a->role }}')"
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-400">Belum ada data admin.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="px-6 py-4 border-t border-gray-200 dark:border-white/[0.05]">
        <p class="text-sm text-gray-500 dark:text-gray-400">Total: {{ $admins->count() }} akun</p>
    </div>
</div>
...
<div id="modalTambah" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
        <div class="flex items-center justify-between mb-5">
            <h4 class="text-lg font-semibold text-gray-800 dark:text-white">Tambah Admin</h4>
...
<div id="modalEdit" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
        <div class="flex items-center justify-between mb-5">
            <h4 class="text-lg font-semibold text-gray-800 dark:text-white">Edit Admin</h4>
...
@push('scripts')
<script>
function editAdmin(id, name, email, noTelp, role) {
    document.getElementById('formEdit').action   = `/admin/admin/${id}`;
