...
<div class="space-y-6">
    <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div class="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div>
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Data Jadwal Latihan</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400">Kelola jadwal latihan mingguan</p>
            </div>
                ...
                <table class="min-w-full">
                    <thead>
                        <tr class="border-gray-200 border-y dark:border-gray-700">
                            ...
                            <th class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Fokus Latihan</th>
                            <th class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Jumlah Gerakan</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-purple-50 text-purple-600 dark:bg-purple-500/15 dark:text-purple-400">
                                    {{ $item->hari }}
                                </span>
                            </td>
                            <td class="px-4 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $item->fokusLatihan }}</div>
                            </td>
                            <td class="px-4 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-blue-50 text-blue-600 dark:bg-blue-500/15 dark:text-blue-400">
                                    {{ $item->gerakan_count }} gerakan
                                </span>
                            </td>
                            <td class="px-4 py-4 text-sm font-medium text-right whitespace-nowrap">
                                <div class="flex justify-center relative">
                                    <x-common.table-dropdown>
                                        ...
                                        <x-slot name="content">
                                            <a href="{{ route('admin.jadwal-latihan.detail', $item->idJadwal) }}"
                                            ...
                                            <button onclick="editJadwal({{ $item->idJadwal }}, '{{ $item->hari }}', '{{ $item->fokusLatihan }}')"
                                                class="flex w-full px-3 py-2 font-medium text-left text-gray-500 rounded-lg text-theme-xs hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5">
                                                Edit
                                            </button>
                                            <form action="{{ route('admin.jadwal-latihan.destroy', $item->idJadwal) }}" method="POST"
                                                onsubmit="return confirm('Yakin? Semua gerakan dalam jadwal ini akan ikut terhapus.')">
                                                @csrf @method('DELETE')
                                                <button type="submit"
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-white/[0.05]">
            <p class="text-sm text-gray-500 dark:text-gray-400">Total: {{ $jadwal->count() }} jadwal</p>
        </div>
    </div>
</div>
...
<div id="modalTambah" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
        <div class="flex items-center justify-between mb-5">
            <h4 class="text-lg font-semibold text-gray-800 dark:text-white">Tambah Jadwal Latihan</h4>
            <button onclick="document.getElementById('modalTambah').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">✕</button>
        </div>
        <form action="{{ route('admin.jadwal-latihan.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fokus Latihan</label>
                <input type="text" name="fokusLatihan" required placeholder="contoh: Chest & Triceps"
                    class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
...
<div id="modalEdit" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
        <div class="flex items-center justify-between mb-5">
            <h4 class="text-lg font-semibold text-gray-800 dark:text-white">Edit Jadwal Latihan</h4>
        ...
        <form id="formEdit" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hari</label>
                <select id="editHari" name="hari" required
                    class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @foreach(['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu','Minggu'] as $hari)
                    <option value="{{ $hari }}">{{ $hari }}</option>
                    @endforeach
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fokus Latihan</label>
                <input type="text" id="editFokus" name="fokusLatihan" required
                    class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div class="flex justify-end gap-3 pt-2">
...
@push('scripts')
<script>
function editJadwal(id, hari, fokus) {
    document.getElementById('formEdit').action = `/admin/jadwal-latihan/${id}`;
    document.getElementById('editHari').value  = hari;
    document.getElementById('editFokus').value = fokus;
